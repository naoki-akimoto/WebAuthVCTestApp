...
@interface KiiUser(IdentityProviderLogin)

/** Authenticate using identity provider by token integration. This is blocking method.
 ...
 @return logged in KiiUser instance, nil if login is failed.
 */
+ (nullable KiiUser*) loginWithProviderSynchronous:(KiiTokenProvider *)provider credential:(NSObject<KiiCredential> *) credential error:(NSError *__autoreleasing  _Nullable *)error;

/** Authenticate using identity provider by token integration. This is non-blocking method.

     [KiiUser loginWithProvider:[KiiFacebookProvider provider]
                    credentials:[KiiOauth2AccessToken accessToken:@"fb_access_token"]
                          block:^(KiiUser *user, NSError *error) {
         if(error == nil) {
             NSLog(@"Authenticated user: %@", user);
         }
 ...
 */
+ (void) loginWithProvider:(KiiTokenProvider *)provider credential:(NSObject<KiiCredential> *) credential block:(KiiUserBlock)block;


/**  Authenticate using webauth provider. This is non blocking method.

     [KiiUser loginWithWebAuth:[KiiOpenIDConnect provider]
                         block:^(KiiUser *user, NSError *error) {
         if(error == nil) {
             NSLog(@"Authenticated user: %@", user);
         }
...
/** Link logged user with Identity provider through token integration. This is non-blocking method
     ...
     [aUser linkWithProvider:[KiiFacebookProvider provider]
                 credentials:[KiiIdentityAccessToken accessToken:@"fb_access_token"]
                       block:^(KiiUser *user, NSError *error) {
         if(error == nil) {
             NSLog(@"Authenticated user: %@", user);
         }
...
/** Unlink current logged user from linked Identity provider. This is blocking method
     ...
     [aUser unlink:[KiiFacebookProvider provider]
             block:^(KiiUser *user, NSError *error) {
         if(error == nil) {
             NSLog(@"Authenticated user: %@", user);
         }
 ...
 */
- (nullable NSDictionary< NSObject<KiiIdentityProvider> *, KiiIdentityInfo *> *) linkedIdentityProviders;
